<template>
  <div class="the-talent">
    <div class="left">
      ...
      <label class="student-name"><strong>{{ theTalent.loginName}}</strong></label>
      <p class="description">there is the description.there is the description.there is the description.there is the description.</p>
      <!-- <p class="description">{{ theTalent.introduction }}</p> -->
      <label>{{ theTalent.followerCount }}</label>
      <label>followers</label>
      <svg-icon icon="follow" style="width: 15px; height:15px;" class="follow-people" @click.native="goToFollow(theTalent.id)" id="follow-people"></svg-icon>
    </div>
    <div class="right">
      <Swiper v-if="theTalent.worksVOList.length > 0" :autoPlay='true' :showIndicator='true' interval="50000" duration="50000">
