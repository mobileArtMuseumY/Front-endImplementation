      ...
      <div class="content">
        ...
        <div class="check-box">
          <el-checkbox v-model="checked">
            <router-link to="/signUpAgreement">同意该协议</router-link>
          </el-checkbox>
        </div>
 ...
 *   3. 表单的准确验证                                                //未解决
 */
import { enterpriseSignUpC } from "@/api/user";
  ...
  methods: {
    ...
    getCaptcha() {
      if (this.ruleForm.phoneNumber) {
        const data = {
          tel: this.ruleForm.phoneNumber
        };
        // 请求服务器发送验证码
        enterpriseSignUpC(data)
          .then(res => {
            this.$message({
              type: "success",
              message: "验证码已发送至您的手机~,请注意查收^_^"
            });
          })
          .catch(err => {
    ...
    goToSignUp() {
      ...
      this.$store.dispatch("VerifyCaptcha", data);
    }
  }
};
</script>
